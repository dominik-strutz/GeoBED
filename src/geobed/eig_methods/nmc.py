...
def nmc(
    ...
    if self.nuisance_dist is None:
        ...
        raise ValueError("NMC not implemented yet for nuisance parameters")
    ...
    if not memory_efficient:
        if not reuse_M:
            marginal_lp = M_data_likelihoods.log_prob(N_data_samples).logsumexp(0) - math.log(M)
        else:
            marginal_lp = M_data_likelihoods.log_prob(
                N_data_samples.unsqueeze(0).swapaxes(0, 1)
                ).swapaxes(0, 1).logsumexp(0) - math.log(M)
    if memory_efficient:
        if reuse_M:
            marginal_lp = torch.zeros(N)
            for i in range(N):
                marginal_lp[i] = M_data_likelihoods.log_prob(N_data_samples[i]).logsumexp(0) - math.log(M)
